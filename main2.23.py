...
def resetSesDict(configProps, tempfolder):
    global maxSesDictPriority, sessionsDict
    maxSesDictPriority = 1
    ...
    print ("sessionsDict was reseted")
...
def PrintBannerAndWarnings():
    ...
    print()
    print("        *******************************************")
    print("        *        ENVIRO MULTISENDER 2.2 (7.5)     *")
...
if __name__ == "__main__":
    ...
    while (continueFlag):
        ...
        for elem in sessionsDict.keys():
            ...
            ftpExceptIP = currentSession.ip + "-" + currentSession.port  # if add user here -will be delay every bad ip using

            if (ftpExceptIP in ftpExceptIParr):
                ## if host was not reachable then do not send it
                logging.info(
                    "," + currentSession.ip + "," + currentSession.user + "," + currentSession.sourcefolder + "," + "skiped")
            ...
            #  ftpExceptIParr.remove(ftpExceptIP)
                ...
                if numsent > 0:
                    logging.info(
                        "," + currentSession.ip + "," + currentSession.user + "," + currentSession.sourcefolder + "," + str(
                            numsent))
                    print("  ", numsent, " files were sent to ", currentSession.ip, currentSession.user)

                elif numsent < 0:  # if there was an exception on   sendFolderFiles(currentSession)
                    ...
                    logging.info(
                        "," + currentSession.ip + "," + currentSession.user + "," + currentSession.sourcefolder + "," + exceptionType)
        ...
        time.sleep(0.05)
        lastRunTime0 = doEveryXmin1Arg(lastRunTime0, 1.2, function=RemoveAllIPfromExeptArr,
                                       arg1=configProps )  # lastRunTime0 will be used as arg on next iteration
        ...
        lastRunTime5 = doEveryXmin2Arg(lastRunTime5, 12, function=resetSesDict,
                                       arg1=configProps,
                                       arg2=tempfolder)
